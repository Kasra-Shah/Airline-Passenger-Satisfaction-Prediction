...
#-----ROC plot----------------------------------------------------
#ROC plot:
#calculating tpr and fpr for roc plot:
tpr_list = list()
for (i in 1:length(cutoff_list)) {
  tpr_list[i] = confusion_matrix_list[[i]][2,2]/sum(confusion_matrix_list[[i]][2,])
}

fpr_list = list()
for (i in 1:length(cutoff_list)) {
  fpr_list[i] = confusion_matrix_list[[i]][2,1]/sum(confusion_matrix_list[[i]][2,])
}
#plotting ROC:
roc_data = data.frame(tpr = unlist(tpr_list), fpr = unlist(fpr_list))
roc_data %>%
  ggplot(aes(x=fpr))+
  geom_line(aes(y=tpr))+
  labs(title = "ROC plot")

final_glm_model = glm(satisfaction ~ . , data = airline_train, family = binomial)
final_glm_pred=list(ifelse(predict(final_glm_model,airline_test,type = "response")>0.55,"satisfied","neutral or dissatisfied"))
final_glm_misclas = calc_misclass(actual = airline_test$satisfaction, predicted = unlist(final_glm_pre))
...
#calculating missclassification for all models:
glm_missclass_list = list()
for (i in 1:length(cutoff_list)) {
  glm_missclass_list[i] = calc_misclass(airline_valid$satisfaction,predict_list[[i]])
}

#plotting missclassifications:
glm_misclas_plot_df = data.frame(cutoff = cutoff_list,missclass = unlist(glm_missclass_list))
glm_misclas_plot_df %>%
  ggplot(aes(x = cutoff, y = missclass))+
  geom_line()
#-----LDA-------------------------------------------------
#LDA:
#fitting lda models:
...
#QDA:
#fitting qda models:
#interacted model could not be fitted becaus of lack of records.
#(model is too complicated to be fitted with this amount of data)
#interacted_qda_model = qda(as.formula(paste('satisfaction ~.^2+',paste('poly(',colnames(airline_est[-c(1,2,4,5,23)]),',2)',collapse = ' + '))),data = airline_est)
qda_model_list = list(
...
tree_model = rpart(satisfaction ~ . , data = airline_est)
tree_predict = predict(tree_model,airline_valid,type = "class")
tree_misclas = calc_misclass(airline_valid$satisfaction,tree_predict)
